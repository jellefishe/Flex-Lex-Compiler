  ...
  main:
	...
	# _tmp3 = LCall _test
	  sw $a2, -12($fp)	# spill s from $a2 to $fp-12
	  jal _test          	# jump to function
	  move $a0, $v0		# copy function return value from $v0
	  lw $a2, -12($fp)	# fill s to $a2 from $fp-12
	# PopParams 8
  ...
  _test:
	...
	# _tmp4 = a + b
	  add $a0, $a1, $a0	
	# Return _tmp4
	  move $v0, $a0		# assign return value into $v0
	  move $sp, $fp		# pop callee frame off stack
	  lw $ra, -4($fp)	# restore saved ra
	  lw $fp, 0($fp)	# restore saved fp
	  jr $ra		# return from function
